...
dir.create(paste0('IDCA_Analysis/Expression_Aware_Cell_Assignment'), recursive = TRUE)
...
for (S in 1:length(listfiles))
{
  ...
  estimate <- fread(paste0(paste0(InputDir, '/SAVER/'), listfiles[S]), stringsAsFactors = FALSE, header = TRUE)
  ...
  {
    merged_Pair <- fread( paste0(InputDir, '/IDCA_Analysis/IDCA_Plots/',S0,'_merged.txt'), stringsAsFactors = FALSE, header = TRUE)
    ...
    vireo <- fread(paste0(InputDir,'/donor_ids.tsv'), stringsAsFactors = FALSE, header = TRUE)
    ...
    {
      freq_c1 <- data.frame(table(merged[which(merged$predicted_clusters == 1), "best_singlet"]), stringsAsFactors = FALSE)
      ...
      freq_c2 <- data.frame(table(merged[which(merged$predicted_clusters == 2),"best_singlet"]), stringsAsFactors = FALSE)
      ...
      #----------------------------------
      fwrite(merged, paste0('IDCA_Analysis/Expression_Aware_Cell_Assignment/', FC, '_', S1, '_', S2, '_Expression_Aware_Cell_Assignment.txt'), row.names = FALSE, quote = FALSE, sep = '\t')
      #----------------------------------
