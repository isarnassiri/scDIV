...
EADDonorPair <- function(InputDir)
{
  ...
  dir.create(paste0('IDCA_Analysis/Expression_Aware_Cell_Assignment'),
             recursive = TRUE)
  ...
  for (S in 1:length(listfiles))
  {
    ...
    estimate <-
      fread(paste0(paste0(InputDir, '/SAVER/'), listfiles[S]),
    ...
    if (file.exists(paste0(InputDir, 'IDCA_Analysis/IDCA_Plots/', S0, '_merged.txt')))
    {
      ...
      {
        freq_c1 <-
          data.frame(table(merged[which(merged$predicted_clusters == 1), "best_singlet"]), stringsAsFactors = FALSE)
        freq_c1 <- freq_c1[order(freq_c1$Freq, decreasing = TRUE), ]
        
        merged[which(merged$predicted_clusters == 1), "predicted_clusters"] <-
          as.character(freq_c1$Var1[1])
        
        freq_c2 <-
          data.frame(table(merged[which(merged$predicted_clusters == 2), "best_singlet"]), stringsAsFactors = FALSE)
        freq_c2 <- freq_c2[order(freq_c2$Freq, decreasing = TRUE), ]
        
        merged[which(merged$predicted_clusters == 2), "predicted_clusters"] <-
          as.character(freq_c2$Var1[1])
        ...
        fwrite(
          ...
          paste0(
            ...
            '_',
            S2,
            '_Expression_Aware_Cell_Assignment.txt'
          ),
          row.names = FALSE,
          quote = FALSE,
          sep = '\t'
        ...
        print(paste0(S, " had one predicted cluster."))
      }
    }
  }
