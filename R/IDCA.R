...
IDCA <- function(InputDir, Donors, FC, ERP, TEST = FALSE)
{
  ...
  if (!TEST)
  {
    for (i in 1:dim(estimate)[1]) {
      ...
      AllGenes <-
        row.names(estimate)[-which(row.names(estimate) == SelectedGene)]
      ...
      y <-
        as.numeric(estimate[which(row.names(estimate) == SelectedGene), ])
      ...
      results <-
        as.data.frame(Lasso_coefficient[which(0 != Lasso_coefficient[, 1]), 1])
      ...
      if (length(AllGenes) > 0)
      {
        for (j in 1:length(AllGenes))
        {
          ...
          if (length(considered_GenePairs[which(considered_GenePairs %in% c(
          ...
          ))]) > 0)
          {
            ...
            considered_GenePairs[length(considered_GenePairs) + 1] <-
              paste(Gene1, Gene2, sep = '_')
            considered_GenePairs[length(considered_GenePairs) + 1] <-
              paste(Gene2, Gene1, sep = '_')
            #----------------------------------
            ...
            estimate_Gene1 <-
              estimate[which(row.names(estimate) == Gene1), ]
            estimate_Gene2 <-
              estimate[which(row.names(estimate) == Gene2), ]
            
            #---------------------------------- Mixture Modeling
            input_mixture_model <-
              t(rbind(
                as.matrix(estimate_Gene1),
                as.matrix(estimate_Gene2)
              ))
            input_mixture_model <-
              as.data.frame(input_mixture_model, stringsAsFactors = FALSE)
            
            mixed_model <-
              flexmix(
                cbind(
                  as.numeric(input_mixture_model[, 1]),
                  as.numeric(input_mixture_model[, 2])
                ) ~ 1,
                k = 2,
                data = input_mixture_model,
                model = FLXMCmvnorm(diag = TRUE),
                #diag = TRUE to get fix results FLXMCmvpois(), FLXMCmvnorm
                control <-
                  list(tolerance = 1e-15, iter.max = 1000)
              )
            ...
            if (length(unique(clusters(mixed_model))) == 2)
            {
              #---------------------------------- IDCA
              design_matrix <-
                cbind(colnames(estimate), colnames(estimate))
              design_matrix[-which(design_matrix[, 1] %in% S1_BARCODE), 1] =
                0
              design_matrix[which(design_matrix[, 1] %in% S1_BARCODE), 1] =
                1
              design_matrix[-which(design_matrix[, 2] %in% S2_BARCODE), 2] =
                0
              design_matrix[which(design_matrix[, 2] %in% S2_BARCODE), 2] =
                1
              ...
              ddcor_G1_G0 <-
                ddcorAll(
                  inputMat = estimate[which(row.names(estimate) %in% c(Gene1, Gene2)), ],
              ...
              input_mixture_model$real_clusters <-
                colnames(estimate)
              input_mixture_model$real_clusters[which(input_mixture_model$real_clusters %in%
                                                        S1_BARCODE)] <- 1
              input_mixture_model$real_clusters[which(input_mixture_model$real_clusters %in%
                                                        S2_BARCODE)] <- 2
              
              input_mixture_model$predicted_clusters <-
                clusters(mixed_model)
              #----------------------------------
              
              #---------------------------------- revise of cluster names
              if (input_mixture_model$predicted_clusters[1] == 2)
              {
                input_mixture_model$predicted_clusters[which(input_mixture_model$predicted_clusters == 1)] <-
                  0
                input_mixture_model$predicted_clusters[which(input_mixture_model$predicted_clusters == 2)] <-
                  1
                input_mixture_model$predicted_clusters[which(input_mixture_model$predicted_clusters == 0)] <-
                  2
              }
              #----------------------------------
              ...
              TPrate <-
                (table[1, 1] + table[2, 2]) / length(clusters(mixed_model))
              ...
              RESULT <-
                RESULT[which(RESULT$pValDiff_adj < 1e-10), ]# & RESULT$G0_pVal != 0.0
        ...
        if (dim(RESULT_merged)[1] != 0)
        {
          ...
          write.table(
            RESULT_merged,
            paste0(OutPutPath, '/', S0, '-', Gene1, '_IDCA.txt'),
            quote = FALSE,
            row.names = FALSE,
            sep = '\t'
          )
