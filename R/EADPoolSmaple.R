...
for(i in 1:length(FCs))
{
  ...
  {
    temp = fread(listfiles_subet[t], select = c('BARCODE', 'predicted_clusters', 'donor_id', 'best_singlet', 'best_doublet', 'Donor1', 'Donor2', 'FC'), stringsAsFactors = F, header = T)
  ...
  for (n in Donors)
  {
    ...
    merged_Pair_subset = merged_Pair_subset[order(merged_Pair_subset$NumDonors, decreasing = T),]
  ...
  fwrite(RESULT, paste0(path, FC, '_Results_Expression_Aware_Cell_Assignment.txt'), quote = F, row.names = F, sep = '\t')
  #----------------------------------
}
