...
path = paste0(InputDir, '/IDCA_Analysis/Expression_Aware_Cell_Assignment/')

#---------------------------------- find the input file
listfiles <- list.files(path, pattern = '_Expression_Aware_Cell_Assignment.txt')

if(length(grep('_Results_Expression_Aware_Cell_Assignment.txt', listfiles))>0)
{
  listfiles <- listfiles[-grep('_Results_Expression_Aware_Cell_Assignment.txt', listfiles)]
}
#----------------------------------
...
for(i in 1:length(FCs))
{
  ...
  {
    temp <- fread(listfiles_subet[t], select = c('BARCODE', 'predicted_clusters', 'donor_id', 'best_singlet', 'best_doublet', 'Donor1', 'Donor2', 'FC'), stringsAsFactors = FALSE, header = TRUE)
    if(t==1){merged_Pair <- temp}else{merged_Pair <- rbind(merged_Pair,temp)}
  }
  ...
  for (n in Donors)
  {
    ...
    merged_Pair_subset <- merged_Pair[which(merged_Pair$Donor1 == n | merged_Pair$Donor2 == n),]
    #----------------------------------
    
    #---------------------------------- remove duplicated cells
    merged_Pair2 <- merged_Pair_subset %>% group_by(BARCODE, sort = TRUE) %>% distinct() %>% count(BARCODE, sort = TRUE)
    ...
    colnames(merged_Pair_subset)[which(colnames(merged_Pair_subset)=='n')] <- 'NumCells'
    
    merged_Pair2 <- merged_Pair_subset %>% group_by(BARCODE, sort = TRUE) %>% distinct() %>% count(predicted_clusters, sort = TRUE) 
    merged_Pair2 <- as.data.frame(merged_Pair2)
    
    merged_Pair_subset <- merge(merged_Pair_subset, merged_Pair2[,c('BARCODE','n')], by = 'BARCODE')
    colnames(merged_Pair_subset)[which(colnames(merged_Pair_subset)=='n')] <- 'NumDonors'
  
    merged_Pair_subset <- merged_Pair_subset[order(merged_Pair_subset$NumDonors, decreasing = TRUE),]
    ...
    merged_Pair_subset$EA_Assignment[which(merged_Pair_subset$NumDonors >= (unique(merged_Pair_subset$NumCells)-1) )] <- 'Assigned'
    merged_Pair_subset$EA_Assignment[which(merged_Pair_subset$NumDonors < (unique(merged_Pair_subset$NumCells)-1) )] <- 'unAssigned'
    
    #---------------------------------- save results
    if(iter==1){RESULT <- merged_Pair_subset; iter <- iter+1;}else{RESULT <- rbind(merged_Pair_subset,RESULT)};
    #----------------------------------
  }
  ...
  print(paste0('% of assigned cell: ', round(dim(RESULT_subset)[1]/dim(RESULT)[1], digits = 3 )*100))
  print(paste0('% of unassigned cell: ', round(1-(dim(RESULT_subset)[1]/dim(RESULT)[1]), digits = 3 )*100))
  ...
  cat('----- Mixed -----',file=paste0(path, FC, '_Summary.txt'),append=TRUE)
  cat("\n",file=paste0(path, FC, '_Summary.txt'),append=TRUE)
  cat(paste0('% of assigned cell: ', round(dim(RESULT_subset)[1]/dim(RESULT)[1], digits = 3 )*100),file=paste0(path, FC, '_Summary.txt'),append=TRUE)
  cat("\n",file=paste0(path, FC, '_Summary.txt'),append=TRUE)
  cat(paste0('% of unassigned cell: ', round(1-(dim(RESULT_subset)[1]/dim(RESULT)[1]), digits = 3 )*100),file=paste0(path, FC, '_Summary.txt'),append=TRUE)
  cat("\n",file=paste0(path, FC, '_Summary.txt'),append=TRUE)
  ...
  fwrite(RESULT, paste0(path, FC, '_Results_Expression_Aware_Cell_Assignment.txt'), quote = FALSE, row.names = FALSE, sep = '\t')
  #----------------------------------
}

cat(paste0("\033[0;", 47, "m", "You can find the results in: ", "\033[0m","\n", InputDir, "/IDCA_Analysis/Expression_Aware_Cell_Assignment/"), paste0(": ", FC, '_Summary.txt & ', paste0(FC, '_Results_Expression_Aware_Cell_Assignment.txt')))
