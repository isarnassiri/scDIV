...
for(i in 1:length(FCs))
{
  ...
  for (n in Donors)
  {
    ...
    colnames(merged_Pair_subset)[which(colnames(merged_Pair_subset)=='n')] = 'NumDonorPairs'
    
    merged_Pair2 = merged_Pair_subset %>% group_by(BARCODE, sort = T) %>% distinct() %>% count(predicted_clusters, sort = T) 
    merged_Pair2 = as.data.frame(merged_Pair2)
    
    merged_Pair_subset = merge(merged_Pair_subset, merged_Pair2[,c('BARCODE','n')], by = 'BARCODE')
    colnames(merged_Pair_subset)[which(colnames(merged_Pair_subset)=='n')] = 'NumAssignedtoDonor'
    ...
    merged_Pair_subset$EA_Assignment[which(merged_Pair_subset$NumAssignedtoDonor >= (unique(merged_Pair_subset$NumDonorPairs)-1) )] = 'Assigned'
    merged_Pair_subset$EA_Assignment[which(merged_Pair_subset$NumAssignedtoDonor < (unique(merged_Pair_subset$NumDonorPairs)-1) )] = 'unAssigned'
