...
for (i in 1:length(Pairs)) 
{
  ...
  fwrite(merged_Pair, paste0('IDCA_Plots/', Pairs[i],'_merged.txt'), quote = FALSE, row.names = FALSE, sep = '\t')
  
  #---------------------------------- read in gene expression profile
  estimate <- fread(paste0(InputDir, '/SAVER/', Pairs[i], '_AssignedCells.txt'), stringsAsFactors = FALSE, header = TRUE)
