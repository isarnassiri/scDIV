...
|:--:| 
| *Figure 7. Example of output from `EADDonorPair()` function. For an indicated pair of the donor (columns Donor1 and Donor2), EADDonorPair() uses a top differentially correlated gene pair (e.g., XIST and RPS27) and using gaussian mixture model clustering, trained by the results of genetics demultiplexing, assigns each cell (BARCODE column) to a donor (predicted_clusters column). In addition, we include the results of genetic demultiplexing (donor_id, prob_max, prob_doublet, n_vars, best_singlet, best_doublet) and flow cell ID (FC).* |
